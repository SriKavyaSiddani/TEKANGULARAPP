...
export class CartComponent implements OnInit {
  ...
  totalPrice:any = 0
  faTrash=faTrash
  constructor(private router : Router ,private kavya : KavyaService , private loader: NgxUiLoaderService) {
    ...
    var body = {}
    this.kavya.getCartItems(url,body,options).subscribe({
      next:(response:any)=>{
   ...
   removeCakeFromCart(index:any){
      ...
      this.loader.start()
      this.kavya.removeCakeFromCart(url,body,options).subscribe({
        next:(response:any)=>{
          this.loader.stop()
   ...
   increaseQuantity(index:any){
     ...
     this.loader.start()
     this.kavya.addCakeToCart(url,this.cartitems[index],options).subscribe({
       next:(response:any)=>{
         this.loader.stop()
         if(response.data){
   ...
   checkout(){
    //  this.kavya.cartitems = this.cartitems
    //  this.kavya.price = this.totalPrice
